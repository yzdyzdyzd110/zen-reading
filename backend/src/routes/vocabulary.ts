...
import { getVocabSets } from '../dataStore.js';
...
router.get('/sets', (req: Request, res: Response) => {
  ...
  const filtered = lang
    ? allVocab.filter((s) => (s as any).language === lang)
    : allVocab;
...
router.get('/sets/:id', (req: Request, res: Response) => {
  const set = getVocabSets().find((s) => s.id === Number(req.params.id));
  if (!set) {
